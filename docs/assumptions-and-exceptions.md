...
7. Crew-facing Talk uses the browser Web Speech API, then a page-field extractor that only fills fields on the current step. Uncertain values stay blank. `/api/speech/transcribe` mock/Azure adapters remain for server-side transcription. Azure Speech is used when `SPEECH_PROVIDER=azure`. Azure OpenAI can replace extraction when `AI_PROVIDER=azure-openai` and credentials exist.
